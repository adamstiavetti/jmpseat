...
| Security events trust boundary | patch pending runtime apply | `docs/ops/security-events-trust-boundary-fix.md`, `src/lib/securityEvents/server.ts`, `supabase/migrations/20260608201541_harden_security_events_trust_boundary.sql`, `test/security-events/securityEventsTrustBoundary.test.mts` | The pending patch removes direct authenticated inserts into trusted `security_events`, marks legacy rows as unverified, and filters operator/admin audit views to trusted server-produced rows. Migration apply, deploy, and runtime validation remain pending after review/merge. |
