...
- No community functionality, AI product feature, payments, or analytics SDK
  ...
  Duplicate survey-token hardening is ready for reviewed runtime migration and
  deploy validation so anonymous duplicate submissions cannot edit another
...
- Duplicate survey-token hardening is implemented for review in
  `ops/waitlist-duplicate-survey-token-fix.md`.

Pending:

- Record first trusted-contact feedback.
- Apply and runtime-validate duplicate survey-token hardening migration before
  deploying the app code; app-first rollout intentionally fails closed rather
  than trusting legacy token-only signup RPC responses.
