...
- No community functionality, AI product feature, payments, or analytics SDK
  exists. First-party waitlist capture is runtime-proven, and the operator/admin
  waitlist dashboard is implemented and runtime-proven. Aggregate metrics stay
...
| 05B | Community Access Architecture | Pending implementation | Define and implement the First-Base MVP launch path under the new email-verification direction: DFW as the first launch base and DFW Base Board as the first available board, inside a model that supports many bases, base boards, layover boards, Verified Lounges / restricted role-based spaces, following, posting, comments, saves, reactions/useful marks, and access-aware search over time. | `strategy/product-pivot-email-verification-community-boards.md`, `strategy/airline-email-access-gate-decision.md`, `strategy/board-community-access-model-decision.md`, `strategy/proof-system-freeze-deprecation-plan.md`, `strategy/first-base-mvp-scope.md`, `strategy/community-admin-responsibilities-disclaimer-policy.md`, `strategy/launch-readiness-gate-transition-plan.md`, `strategy/beta-invite-code-foundation-decision.md`, `ops/05b-first-base-mvp-planning.md`, `epochs/first-base-mvp-implementation-ticket-pack.md`, `epochs/fbmvp-t01-freeze-user-facing-proof-verification-surfaces.md`, `epochs/fbmvp-t02-airline-email-verification-access-state-design.md`, `epochs/fbmvp-t02-airline-email-verification-access-state-implementation.md`, `epochs/fbmvp-t03-private-testing-versus-first-base-launch-gate-implementation.md`, `epochs/fbmvp-t03a-beta-invite-code-foundation-implementation.md`, `epochs/fbmvp-t04-onboarding-signup-flow-update.md`, `ops/beta-invite-code-foundation-runtime-pass.md`, `ops/private-beta-readiness-bridge.md`, `ops/auth-design-system-style-guide.md`, `ops/auth-design-overhaul-docs-audit.md`, `ops/auth-email-branding-confirmation-template-plan.md`, `MVP_SCOPE.md`, `BUILD_TICKETS.md`, `FEATURE_ROADMAP.md`, `DISCOVERY_RESEARCH_REPORT_001.md`, `SCALABILITY_AND_ARCHITECTURE_REQUIREMENTS.md`, `MOBILE_APP_STRATEGY.md` | Docs define the First-Base MVP lane, with `ops/05b-first-base-mvp-planning.md` as the controlling entry note. The first implementation ticket is model-aware `FBMVP-T05`, followed by follows/home-base preferences, restricted lounge access/community-admin foundations, DFW Base Board shell, board/layover discovery, text posts/comments, saves/reactions/useful/trending, access-aware search, moderation/admin controls, and seeded DFW launch validation. | Implementation blockers remain: first launch base confirmation, included airline domains, MVP restricted lounges, moderation scope, first community-admin appointment owner, and canonical jmpseat reaction terminology. `FBMVP-T01` through `FBMVP-T04` are already implemented, proof uploads remain deprecated/out of current scope, self-declared profile fields must not become authorization truth, airline-email verification grants broad app eligibility only, and community-admin grants must remain separate from operator grants. | Start 05B with `FBMVP-T05: Base, Board, And Board-Type Data Model Design`, using `ops/05b-first-base-mvp-planning.md` as the controlling scope note. |
