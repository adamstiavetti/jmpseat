...
Epochs remain the strategic planning model, but current implementation truth is
resolved in this order:

1. current repo/code on the active branch
2. current focused checkpoint, milestone, and ops docs
3. this epoch roadmap and other broad roadmap docs
4. older broad docs as historical or V1 context

When broad roadmap docs and focused checkpoint docs conflict, the focused
checkpoint/current milestone docs win until the broad docs are updated.
...
- Product direction has pivoted: forward app-level access should use confirmed approved airline employee email control, and restricted role/base board access should be community-admin managed rather than based on jmpseat proof-upload review. Proof upload is frozen as a forward product path, First-Base MVP scope is defined as the first complete base launch package, community-admin responsibilities/disclaimers are defined, the private-testing-to-first-base-launch gate transition is defined, beta invite codes are defined as private-testing capacity control only, and the First-Base MVP implementation ticket pack now defines the forward `FBMVP` sequence. `FBMVP-T01` freezes normal user-facing proof upload surfaces; `FBMVP-T02` adds the airline-email access-state helper/adapter; `FBMVP-T03` adds explicit launch-mode app access gates; `FBMVP-T03A` adds beta invite-code foundation before onboarding/signup work and is runtime-proven on the linked Supabase runtime; `FBMVP-T04` aligns onboarding/signup/access-hold copy with the current access journey; the auth design-system overhaul now makes `/app/access-hold` the canonical airline employee email verification surface and deprecates `/app/verification` as a standalone page; founder/admin internal private-app access now uses explicit operator grants rather than temporary airline-domain abuse, and the private-app override now runtime-provenly requires the dedicated `operator.internal_private_app_access` scope rather than any unrelated operator/admin tooling scope; the security-events trust-boundary fix now runtime-provenly removes direct authenticated writes to the trusted audit table, marks legacy rows unverified, routes server logging through the trusted service-role writer, and filters operator/admin audit views to trusted server-produced rows; proof-upload content validation is deployed and route-smoke-verified at `329d238`, requiring server-side JPEG/PNG byte validation before private proof Storage upload while preserving bucket privacy and reviewer signed-URL controls; proof uploads are now deprecated/out of current scope, and the old live authenticated mutation test is not an active blocker; security-header hardening is runtime-proven at `8558d2d`, adding app-owned `nosniff`, referrer, permissions, anti-framing, and report-only broad CSP headers while leaving Vercel HSTS unchanged; beta Vercel env scoping is now documented and runtime-verified so normal beta Preview deploys have the required persistent Supabase env names without per-deploy injection; Epoch 5 is closed with explicit carry-forward items in `ops/epoch-5-final-closeout.md`, and the immediate post-Epoch-5 narrow lane is now reconciled in `ops/private-beta-readiness-bridge.md`; post-bootstrap operator grant management is implemented and runtime-proven through the linked runtime RPC/action path; app-generated work-email verification codes are implemented with inline access-hold entry and legacy confirm-route compatibility; normal account signup confirmation uses a Supabase Auth-native code-first account confirmation UX and the unified stable-beta auth flow is founder-confirmed at runtime; password reset remains link-driven through the existing recovery flow; the temporary `jmpseat.com` approved-domain workaround has been soft-disabled; auth email branding/custom SMTP is now a deferred beta-readiness TODO for trust/deliverability/polish rather than an auth-flow implementation blocker. See `strategy/product-pivot-email-verification-community-boards.md`, `strategy/airline-email-access-gate-decision.md`, `strategy/board-community-access-model-decision.md`, `strategy/proof-system-freeze-deprecation-plan.md`, `strategy/first-base-mvp-scope.md`, `strategy/community-admin-responsibilities-disclaimer-policy.md`, `strategy/launch-readiness-gate-transition-plan.md`, `strategy/beta-invite-code-foundation-decision.md`, `epochs/first-base-mvp-implementation-ticket-pack.md`, `epochs/fbmvp-t01-freeze-user-facing-proof-verification-surfaces.md`, `epochs/fbmvp-t02-airline-email-verification-access-state-design.md`, `epochs/fbmvp-t02-airline-email-verification-access-state-implementation.md`, `epochs/fbmvp-t03-private-testing-versus-first-base-launch-gate-implementation.md`, `epochs/fbmvp-t03a-beta-invite-code-foundation-implementation.md`, `epochs/fbmvp-t04-onboarding-signup-flow-update.md`, `epochs/founder-admin-private-app-access-implementation.md`, `epochs/operator-grant-management-implementation.md`, `epochs/work-email-confirmation-email-flow-implementation.md`, `epochs/account-signup-code-confirmation-implementation.md`, `ops/beta-invite-code-foundation-runtime-pass.md`, `ops/e05-operator-grant-management-runtime-pass.md`, `ops/operator-private-app-scope-gate-fix.md`, `ops/security-events-trust-boundary-fix.md`, `ops/proof-upload-content-validation-fix.md`, `ops/security-headers-hardening.md`, `ops/beta-vercel-env-scoping.md`, `ops/epoch-5-final-closeout.md`, `ops/private-beta-readiness-bridge.md`, `ops/auth-design-system-style-guide.md`, `ops/auth-design-overhaul-docs-audit.md`, `ops/auth-detour-closeout-runtime-pass.md`, and `ops/auth-email-branding-confirmation-template-plan.md`.
- No AI product feature or payments exist. First-party waitlist capture is
  runtime-proven, and the operator/admin waitlist dashboard is implemented and
  runtime-proven. Aggregate metrics stay
  available through `operator.read_audit`, while raw waitlist contact/detail
  workflow is narrowed behind `operator.view_waitlist_contacts`. Fresh
  first-operator bootstrap should include that narrow scope so a new
  environment still has one usable founder/admin waitlist-contact operator, and
  post-bootstrap operator grant management should be able to grant that scope
  through a controlled path. Root public cutover is complete for the apex
  `jmpseat.com` domain and is documented in
  `ops/public-waitlist-root-cutover-runtime-pass.md`. W05A addresses the prior
  metadata/legal/accessibility launch blockers in code, the public waitlist
  homepage/legal polish is merged, and root waitlist capture plus optional
  survey persistence are runtime-proven. `www.jmpseat.com` is also configured
  and smoke-tested against the same public waitlist deployment as apex.
  Duplicate survey-token hardening is migration-first deployed and
  runtime-proven, so anonymous duplicate submissions cannot edit another
  signup's optional survey answers.
...
- `ops/05b-first-base-mvp-planning.md` is now the controlling note for the
  first 05B implementation start. DFW is the first launch base/board, not the
  whole product concept, and the schema should support many bases, boards,
  follows, lounges, posts, saves, reactions, and access-aware search over time.
- 05B / First-Base MVP is active and in progress, not merely pending. The
  current checkpoint is `c2bbd73 docs: record dfw hub channels checkpoint`.
  T25B and T26A are implemented, committed, runtime-applied, and documented:
  runtime now has the `hub_channel` board type, six DFW child channel boards,
  and `public.list_open_hub_channels(p_base_code text)`, plus the protected
  `/app/hubs/dfw/channels` route in code.
- The current DFW Hub MVP is four utility pillars: DFW Today, Base, Layover,
  and Channels. Channels is the current implementation lane, not the entire
  MVP. DFW Today/Base/Layover still need lightweight functional baselines
  before narrow private-beta MVP.
- Authenticated browser smoke for `/app/hubs/dfw/channels` remains pending.
  Stale test cleanup also remains pending for
  `test/community/hubChannelSeeds.test.mts`,
  `test/community/boardPostActions.test.mts`, and
...
| 01 | Public Splash + Waitlist | Active / public domains launched | Make concept publicly viewable and capture waitlist interest. | `APP_FOUNDATION_NOTES.md`, `LANDING_PAGE_WAITLIST_PLAN.md`, `DEPLOYMENT_AND_WAITLIST_READINESS.md`, `ops/post-e05-public-waitlist-launch-plan.md`, `ops/waitlist-question-research-selection.md`, `ops/public-waitlist-page-polish.md`, `ops/waitlist-metrics-dashboard.md`, `ops/public-waitlist-launch-readiness-check.md`, `ops/public-waitlist-root-cutover-runtime-pass.md`, `ops/public-waitlist-www-runtime-pass.md`, `ops/waitlist-duplicate-survey-token-fix.md`, `DEPLOYMENT_RECORD_001.md` | Public `jmpseat.com` and `www.jmpseat.com` serve the waitlist page without Beta Access, first-party email capture works, optional product-shaping follow-up works for legitimate new signup sessions, mobile page reviewed, first-party waitlist metrics captured, authorized admin metrics view exists, duplicate submissions cannot receive existing survey tokens after migration apply, and launch metadata/legal surfaces are root-ready. | Apex root cutover is runtime-proven; `www.jmpseat.com` DNS/Vercel alias follow-up is smoke-tested; duplicate survey-token hardening is migration-first deployed and runtime-proven. | Monitor root and `www` waitlist reachability while preserving beta; future optional survey editing would require email ownership proof or authenticated account flow. |
...
| 05 | Operator/Admin Tooling Foundation | Closed with carry-forward items | Preserve the completed operator/admin verification foundation while product direction pivots away from proof-upload verification. | `epochs/epoch-05-operator-admin-tooling-tickets.md`, `strategy/product-pivot-email-verification-community-boards.md`, `strategy/airline-email-access-gate-decision.md`, `strategy/board-community-access-model-decision.md`, `strategy/proof-system-freeze-deprecation-plan.md`, `strategy/first-base-mvp-scope.md`, `strategy/community-admin-responsibilities-disclaimer-policy.md`, `strategy/launch-readiness-gate-transition-plan.md`, `strategy/beta-invite-code-foundation-decision.md`, `epochs/first-base-mvp-implementation-ticket-pack.md`, `epochs/fbmvp-t01-freeze-user-facing-proof-verification-surfaces.md`, `epochs/fbmvp-t02-airline-email-verification-access-state-design.md`, `epochs/fbmvp-t02-airline-email-verification-access-state-implementation.md`, `epochs/fbmvp-t03-private-testing-versus-first-base-launch-gate-implementation.md`, `epochs/fbmvp-t03a-beta-invite-code-foundation-implementation.md`, `epochs/fbmvp-t04-onboarding-signup-flow-update.md`, `epochs/founder-admin-private-app-access-implementation.md`, `epochs/operator-grant-management-implementation.md`, `ops/beta-invite-code-foundation-runtime-pass.md`, `ops/e05-operator-grant-management-runtime-pass.md`, `ops/operator-private-app-scope-gate-fix.md`, `ops/security-events-trust-boundary-fix.md`, `ops/proof-upload-content-validation-fix.md`, `ops/security-headers-hardening.md`, `ops/beta-vercel-env-scoping.md`, `ops/epoch-5-final-closeout.md`, `ops/private-beta-readiness-bridge.md`, `ops/auth-design-system-style-guide.md`, `ops/auth-design-overhaul-docs-audit.md`, `ops/auth-email-branding-confirmation-template-plan.md`, `epochs/e05-operator-access-model-decision.md`, `epochs/e05-admin-shell-navigation-foundation.md`, `epochs/e05-operator-grants-foundation.md`, `epochs/e05-approved-domain-management.md`, `epochs/e05-reviewer-scope-management.md`, `epochs/e05-verification-audit-inspection.md`, `epochs/e05-proof-cleanup-monitoring.md`, `epochs/e05-protected-manual-proof-cleanup-controls.md`, `ops/operator-grants-bootstrap-runtime-pass.md`, `ops/approved-domain-management-runtime-pass.md`, `ops/reviewer-scope-management-runtime-pass.md`, `ops/verification-audit-inspection-runtime-pass.md`, `ops/proof-cleanup-monitoring-runtime-pass.md`, `ops/protected-manual-proof-cleanup-controls-runtime-pass.md`, `epochs/epoch-04-exit-report.md`, `BUILD_TICKETS.md`, `SCALABILITY_AND_ARCHITECTURE_REQUIREMENTS.md`, `MOBILE_APP_STRATEGY.md` | Historical operator/admin verification tooling remains documented and runtime-proven; forward planning defines airline-email access, board memberships, community-admin authority, proof-system freeze/deprecation, First-Base MVP scope, community-admin responsibilities/disclaimers, launch-readiness gate transition, beta invite-code private-testing control, auth email branding/manual ops requirements, auth visual-system rules, least-privilege private-app operator override, trusted security-events boundary hardening, deployed historical proof-upload content validation hardening with mutation testing deprecated for the current path, runtime-proven security-header hardening, persistent beta Preview Supabase env scoping, the post-Epoch-5 private beta readiness bridge, and the forward FBMVP ticket sequence. | E05-T01 through E05-T07 are implemented/runtime-proven, `FBMVP-T01` freezes normal user-facing proof surfaces, `FBMVP-T02` adds the airline-email access-state helper/adapter, `FBMVP-T03` adds launch-mode app access gates, `FBMVP-T03A` adds beta invite-code foundation and is runtime-proven on the linked Supabase runtime, `FBMVP-T04` aligns onboarding/signup flow copy with the current gate model, the auth design-system overhaul deprecates `/app/verification` and moves airline employee email verification into `/app/access-hold`, founder/admin internal private-app access reuses explicit operator grants and now runtime-provenly requires the dedicated `operator.internal_private_app_access` scope for app-entry override, the security-events trust boundary is singly migrated, deployed, and runtime-proven, proof-upload content validation is deployed and route-smoke-verified, with live mutation testing deprecated for the current path, security-header hardening is runtime-proven on apex/www/beta, beta Preview env scoping is runtime-verified without deployment-scoped Supabase env injection, post-bootstrap operator grant management is implemented and runtime-proven through the linked runtime RPC/action path, auth email branding/custom SMTP ops planning exists, and additional proof-upload/proof-cleanup expansion is frozen. E05-T08 remains paused unless explicitly reactivated. | Follow `ops/private-beta-readiness-bridge.md` for the narrow post-Epoch-5 lane; proof-upload live mutation testing is no longer a prerequisite before 05B. |
| 05B | First-Base MVP / DFW Hub Utility Foundation | Active / in progress | Implement the narrow DFW Hub private-beta MVP under the post-Hub-pivot model. The DFW Hub MVP has four utility pillars: DFW Today, Base, Layover, and Channels. Channels is the current implementation lane, not the whole MVP. | `ops/05b-first-base-mvp-planning.md`, `ops/hub-pivot-plan.md`, `ops/fbmvp-checkpoint-dfw-hub-channels-foundation-level-set.md`, `ops/fbmvp-remaining-functional-backlog.md`, `BUILD_TICKETS.md`, `DATA_MODEL.md`, `MVP_SCOPE.md`, `FEATURE_ROADMAP.md`, `SCALABILITY_AND_ARCHITECTURE_REQUIREMENTS.md`, `MOBILE_APP_STRATEGY.md` | T25B and T26A are implemented, committed, runtime-applied, and documented. Runtime now has `hub_channel`, six DFW child channel boards, and `public.list_open_hub_channels(p_base_code text)`. Code now has `/app/hubs/dfw/channels`. Narrow MVP still requires authenticated Channels browser smoke, stale test cleanup, channel thread-list/detail/composer/comment/report/moderation work, and lightweight DFW Today/Base/Layover baselines. | Authenticated browser smoke for `/app/hubs/dfw/channels` is pending. Stale test cleanup is pending for `hubChannelSeeds`, `boardPostActions`, and `boardPostDetail`. DFW Today/Base/Layover remain lightweight functional baselines to build. Search, saves, reactions, Request a Channel workflow, AI, deals, media, live integrations, multi-airport, and native mobile/app-store work remain deferred. | Run authenticated `/app/hubs/dfw/channels` browser smoke, clean stale tests, then start `T26B` channel thread-list read foundation. |
...
- Current post-E05 public launch intent is captured in
  `ops/post-e05-public-waitlist-launch-plan.md`.
...
- `/app/admin/waitlist` is implemented as the operator/admin-only waitlist
  ...
  is complete for both masked audit mode and founder-confirmed contact-detail
  mode.
...
- Initial operator/admin waitlist metrics dashboard is implemented and
  runtime-proven.
- Post-E05 domain split and waitlist launch intent.
- W05 launch-readiness audit is documented in
  `ops/public-waitlist-launch-readiness-check.md`, and apex root cutover is
  documented in `ops/public-waitlist-root-cutover-runtime-pass.md`. The
  `www.jmpseat.com` follow-up is documented in
  `ops/public-waitlist-www-runtime-pass.md`.
- Duplicate survey-token hardening is migration-first deployed and
  runtime-proven in `ops/waitlist-duplicate-survey-token-fix.md`.

Pending:

- Record first trusted-contact feedback.
- Continue monitoring root and `www` waitlist reachability while preserving
  beta. App-first rollout remains documented as intentionally fail-closed for
  any future legacy token-only signup RPC response.
...
- **Replace prior cinematic/globe public waitlist direction with simpler premium static `jmpseat.` waitlist**
  ...
  - Requirements:
    ...
    - First-party waitlist capture remains intact.
    ...
    - Must not add auth, verification, community features, AI, payments, or other product features as part of the redesign.
  - Operating rule: this public redesign remains separate from Epoch 03/auth and from the private `/app` shell.

Exit criteria:

- Production loads.
- Public `jmpseat.com` page has no Beta Access entry.
- First-party email capture works.
- Optional product-shaping follow-up works and can be skipped.
- Non-sensitive runtime submission is verified.
- Mobile page reviewed.
- First-party waitlist metrics are captured and viewable by an authorized
  operator/admin.
...
- support the historical worker-verification foundation while current direction
  prioritizes airline work-email verification and non-upload review; redacted
  badge/proof upload is deprecated/out of current scope unless a future reviewed
  plan reactivates it
...
- Continue through the post-E05 public waitlist launch lane in
  ...
  implemented and runtime-proven, W05A addresses the prior metadata/legal
  accessibility blockers in code, the public waitlist homepage/legal polish is
  merged, and apex root cutover is complete with runtime-proven waitlist capture
  before W06 native architecture readiness. Keep
  `ops/auth-email-branding-confirmation-template-plan.md` as the deferred
  Supabase Auth confirmation/reset branding TODO for trust, deliverability, and
  polish.
- Continue product validation outreach in parallel:
  - submit and verify a non-sensitive first-party waitlist response after the
    runtime migration path is approved
...
Recommended next task: continue private beta readiness checklist reconciliation while keeping branded sender/custom SMTP/email template polish as a deferred TODO defined in `ops/auth-email-branding-confirmation-template-plan.md`.
