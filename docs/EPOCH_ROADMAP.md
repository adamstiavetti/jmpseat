...
- No community functionality, AI product feature, payments, or analytics SDK
  ...
  through a controlled path. Root public cutover remains pending behind the W05
  readiness audit in `ops/public-waitlist-launch-readiness-check.md`. W05A
  addresses the prior metadata/legal/accessibility launch blockers in code, the
  public waitlist homepage/legal polish is merged, and the latest readiness
  rerun now marks the lane ready with final manual visual QA before root
  cutover approval.
...
| 01 | Public Splash + Waitlist | Active / ready with final manual visual QA | Make concept publicly viewable and capture waitlist interest. | `APP_FOUNDATION_NOTES.md`, `LANDING_PAGE_WAITLIST_PLAN.md`, `DEPLOYMENT_AND_WAITLIST_READINESS.md`, `ops/post-e05-public-waitlist-launch-plan.md`, `ops/waitlist-question-research-selection.md`, `ops/public-waitlist-page-polish.md`, `ops/waitlist-metrics-dashboard.md`, `ops/public-waitlist-launch-readiness-check.md`, `DEPLOYMENT_RECORD_001.md` | Public `jmpseat.com` serves the waitlist page without Beta Access, first-party email capture works, optional product-shaping follow-up works, mobile page reviewed, first-party waitlist metrics captured, authorized admin metrics view exists, and launch metadata/legal surfaces are root-ready. | Code/docs blockers are cleared; root public cutover still needs the final manual mobile/desktop visual pass on the root launch target and an explicit cutover approval. | Run final manual root-target visual QA, then approve or reject root-domain cutover as a separate task. |
...
- W05 launch-readiness audit is documented in
  `ops/public-waitlist-launch-readiness-check.md`.

Pending:

- Final manual visual QA on the root cutover target.
- Record first trusted-contact feedback.
- Approve or reject root-domain cutover as a separate explicit task after the
  latest readiness rerun.
...
- Continue through the post-E05 public waitlist launch lane in
  ...
  merged, and the latest readiness rerun now marks W05 ready with final manual
  visual QA before any explicit public-domain cutover task or W06 native
