...
- [Security Events Trust Boundary Fix](ops/security-events-trust-boundary-fix.md) - removes direct authenticated inserts into trusted `security_events`, marks legacy audit rows as unverified, and keeps operator/admin audit views filtered to trusted server-produced rows. Migration apply and deploy validation remain pending after review/merge.
