...
- [Post-E05 Public Waitlist Launch Plan](ops/post-e05-public-waitlist-launch-plan.md) - current planning source for the public `jmpseat.com` waitlist release, Tally capture, first-party metrics/admin dashboard, and Vercel/Supabase/Expo architecture direction.
...
- Next implementation work should follow the post-E05 public waitlist launch lane in `ops/post-e05-public-waitlist-launch-plan.md`: polish the public `jmpseat.com` waitlist page, remove/no-show public Beta Access entry, keep `beta.jmpseat.com` as the private beta/auth/admin surface, wire reviewed Tally capture, add first-party waitlist metrics, add an admin metrics viewer, and preserve the future Expo/EAS native path. `FBMVP-T01` through `FBMVP-T04`, auth design-system overhaul, founder/admin internal access, post-bootstrap operator grant management, app-generated work-email verification codes, account signup code confirmation, and auth detour closeout are already implemented or documented in their respective epoch/ops notes. `ops/auth-email-branding-confirmation-template-plan.md` remains the Supabase Auth confirmation/reset branding plan before broader auth entry. Content/moderation policy can still be refined before live community launch if needed.
