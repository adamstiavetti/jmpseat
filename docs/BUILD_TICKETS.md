...
- [Proof Upload Content Validation Fix](ops/proof-upload-content-validation-fix.md) - deployed proof-upload hardening that stops redacted proof upload from trusting browser MIME/extension alone and requires server-side JPEG/PNG byte validation before private Storage upload; beta/public route smoke passed, with live authenticated proof-upload mutation pending until a safe founder-controlled test workflow is available.
...
- [Epoch 5 Final Closeout](ops/epoch-5-final-closeout.md) - closes the bounded operator/admin/security hardening lane with explicit carry-forward items for live proof-upload mutation validation, CSP reporting/enforcement, Vercel deployment-model maturity, Supabase migration-history drift handling, auth email branding/custom SMTP, and the next community-access/moderation lane.
- [Private Beta Readiness Bridge](ops/private-beta-readiness-bridge.md) - reconciles the closed Epoch 5 baseline with the broader private-beta docs, keeps live proof-upload mutation as a carry-forward item, treats auth email branding/custom SMTP as near-term readiness work, and defines the narrow lane before broader 05B / community-access implementation.
...
- [Auth Email Branding / Confirmation Template Ops Plan](ops/auth-email-branding-confirmation-template-plan.md) - current readiness package for branded jmpseat auth emails, covering ownership split across Supabase and repo-owned email flows, sender/domain requirements, callback/redirect expectations, template copy targets, manual dashboard/provider steps, and runtime validation required before broader beta auth entry.
